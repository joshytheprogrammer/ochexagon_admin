    ...
    <div className="w-full h-full overflow-hidden">
      ...
      <div className="h-[75%] mo-lg:h-[90%] scrollable-content overflow-auto">
        <table className="table w-max mo-lg:w-full h-fit text-left mo-lg:text-md">
          ...
          {testimonials.length == 0 ? <div>No result found</div> : <tbody className="w-full font-semibold">
            {testimonials.map((testimonial) => (
              <tr key={testimonial.id}>
                ...
                <td className="p-3">{testimonial.data.testimony}</td>
                <td className="p-3">{testimonial.data.name}</td>
                <td className="p-3"></td>
              </tr>
            ))}
          </tbody>}
